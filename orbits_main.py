...
def closing_animation(fsize=40):
...
def main_surface_text(fsize=20):
    font = pygame.font.Font('freesansbold.ttf', fsize)
...
def status_surface_text(fsize=12):
    font = pygame.font.Font('freesansbold.ttf', fsize)
    text1 = font.render("PLANET", True, WHITE)
    text2 = font.render("SPEED", True, WHITE)
    text3 = font.render("DISTANCE", True, WHITE)
    text_rect1 = text1.get_rect()
    text_rect2 = text2.get_rect()
    text_rect3 = text3.get_rect()
    part = status_surface.get_width() / 3
    text_x1 = 10
    text_x2 = part
    text_x3 = part * 2
    text_y1 = text_y2 = text_y3 = 35
    status_surface.blit(text1, [text_x1, text_y1])
    status_surface.blit(text2, [text_x2, text_y2])
    status_surface.blit(text3, [text_x3, text_y3])

def distance_text(surface, pos, i, fsize=12):
    dis = distance([planet[i]['x'], planet[i]['y']], [screen.get_width()/2, screen.get_height()/2])

    font = pygame.font.Font('freesansbold.ttf', fsize)
    text1 = font.render("{0:.2f}".format(dis), True, WHITE)
    text_rect1 = text1.get_rect()
    text_x1 = pos[0]
    text_y1 = pos[1] - text_rect1.height
    surface.blit(text1, [text_x1, text_y1])

def speed_text(surface, pos, vel=v, fsize=12):
    font = pygame.font.Font('freesansbold.ttf', fsize)
...
# -------- Main Program Loop -----------
def main():
    global done, mouseClicked1, mouseClicked2, planet_color, v, clock, circle_x, circle_y, planet, dir_x, dir_y, theta, alpha, a, t, b_r, p_r, p_m, G, M, eaten, game_over
    while not done:
        ...
        if not eaten:
            ...
            if mouseClicked1 and not mouseClicked2:
                ...
                speed_text(screen, [dir_x, dir_y], v)

            if len(planet) > 0:
                for i in range(0, len(planet)):
                    draw_line(screen, WHITE, [planet[i]['x'], planet[i]['y']], [planet[i]['x'] + 25, planet[i]['y'] - 10])
                    speed_text(screen, [planet[i]['x'] + 25, planet[i]['y'] - 10], planet[i]['velocity'])
                    draw_circle(screen, planet[i]['color'], [planet[i]['x'], planet[i]['y']], 5, True, True)
                    #status surface
                    draw_circle(status_surface, planet[i]['color'], [15, 65+(i*25)], 5, True, True)
                    speed_text(status_surface, [status_surface.get_width()/3, 73+(i*25)], planet[i]['velocity'])
                    distance_text(status_surface, [2*status_surface.get_width()/3, 73+(i*25)], i)
